...
class PostReportPresenter
{
    ...
    public function __construct(Post $post, int $currentWorkspaceId, int $interval)
    {
        $this->messageRepo = app(MessageTenantRepositoryInterface::class);
        $this->messageUrlRepo = app(MessageUrlRepository::class);
        $this->postRepo = app(PostTenantRepositoryInterface::class);
    ...
    {
        if (!$this->post) {
            throw new RuntimeException('Post must be initialised');
        }
    ...
    /**
     * Get all clicked links for a post.
     *
     * @throws Exception
     */
    private function getPostUrls(): Collection
    {
    ...
    /**
     * Get count and ratio statistics for a post.
     */
    private function getPostStats(): array
    {
        $countData = $this->postRepo->getCounts(collect($this->post->id), $this->currentWorkspaceId);

        return [
            'counts' => [
                'open' => (int) $countData[$this->post->id]->opened,
                'click' => (int) $countData[$this->post->id]->clicked,
                'sent' => $this->post->formatCount((int) $countData[$this->post->id]->sent),
                'bounce' => (int) $countData[$this->post->id]->bounced,
            ],
            'ratios' => [
                'open' => $this->post->getActionRatio((int) $countData[$this->post->id]->opened, (int) $countData[$this->post->id]->sent),
                'click' => $this->post->getActionRatio((int) $countData[$this->post->id]->clicked, (int) $countData[$this->post->id]->sent),
                'bounce' => $this->post->getActionRatio((int) $countData[$this->post->id]->bounced, (int) $countData[$this->post->id]->sent),
            ],
        ];
    }
}
